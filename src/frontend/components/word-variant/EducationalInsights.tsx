    ...
    <div className="space-y-8">
        ...
        <div className="p-8 rounded-[2.5rem] bg-amber-500/5 border border-amber-500/10 space-y-4">
          ...
          <div className="space-y-2">
            {variant.tips?.map((tip, i) => (
              <p key={i} className="text-sm text-amber-200/60 leading-relaxed flex gap-2">
                <span className="text-amber-500">•</span> {tip}
              </p>
            )) || (
              <p className="text-sm text-amber-200/40 italic">Focus on the root meaning for better recall.</p>
            )}
          </div>
        </div>
      </div>
      ...
      {variant.commonMistakes && (
        <div className="p-6 rounded-[2rem] bg-red-500/5 border border-red-500/10 flex items-start gap-4">
          ...
          <div>
            <h5 className="text-sm font-black text-red-400 uppercase tracking-widest mb-1">Watch out!</h5>
            <p className="text-sm text-red-200/60 leading-relaxed italic">{variant.commonMistakes}</p>
          </div>
        </div>
      )}
    </div>
