...
export abstract class BaseSlangAnalyzer implements ISlangAnalyzer {
  protected static readonly SUPPORTED_VARIANTS = [
    // American English
  ...
  abstract analyzeSlang(text: string): Promise<SlangData>;

  protected getVariantList(): string {
    return BaseSlangAnalyzer.SUPPORTED_VARIANTS.map(
      (v) => `- ${v.region}${v.subregion ? ` / ${v.subregion}` : ""} (${v.country} ${v.flag})`
    ).join("\n");
  }

  protected enrichVariants(parsed: Partial<SlangData>): SlangData {
    const enriched: RegionalVariant[] = (parsed.regionalVariants ?? []).map((v) => {
      const meta = BaseSlangAnalyzer.SUPPORTED_VARIANTS.find(
  ...
  protected getBasePrompt(text: string): string {
    return `You are a senior sociolinguist and English dialect expert specializing in regional variation across global English varieties.
